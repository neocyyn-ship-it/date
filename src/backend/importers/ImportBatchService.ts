...
export class ImportBatchService {
  ...
  static async commitFiles(filePaths: string[], overrides?: Record<string, string>): Promise<ImportCommitResult[]> {
    await duckDbClient.init();
    ...
    for (const filePath of filePaths) {
      ...
      try {
        ...
        if (preview.sourceType === 'refund_template') {
          ...
          const rows = ExcelImporterService.parseProductRecords(filePath, overrides);
